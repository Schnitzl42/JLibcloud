...
class OpenStack_1_1_NodeDriver(OpenStackNodeDriver):
    ...
    def ex_list_networks(self, tenant_id):
        """
        ...
        print("!========!")
        try:
            print("2.0 api request")
            response = self.connection.request('/v2.0/networks', method='GET').object
        except Exception, e:
            print('failed')
        try:
            print('std-libcloud request')
            response = self.connection.request(self._networks_url_prefix, method='GET').object
        except Exception, e:
            print('failed')
        if tenant_id:
            try:
                print("1.0 api request")
                response = self.connection.request(('/tenants/%s/networks.json')%(tenant_id), method='GET').object
            except Exception, e:
                print('failed')
